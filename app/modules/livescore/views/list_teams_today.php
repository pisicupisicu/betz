...
<div style="width:auto;text-align:center;" align="center">
	...
	<?=$this->dataset->table_head();?>                        
		...
		foreach ($this->dataset->data as $row) {                    
          if (!$row['ok']) {
              $link = '/admincp3/livescore/list_matches_by_team_id_today/' . $row['index'];
              if (!$row['similar_teams']) {
                  $edit = '/admincp3/livescore/edit_team_today/' . $row['index'];
              } else {
                  $edit = '/admincp3/livescore/edit_team_today_similar/' . $row['index'];
              }
